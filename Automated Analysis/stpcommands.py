...
#-------------------------------------------------------
def branch_diff(a, b, c, wordsize):
    command = ""
    command += "ASSERT({} = {});\n".format(a, b)
    command += "ASSERT({} = {});\n".format(a, c)
    return command
def branch_lin(a, b, c, wordsize):
    command = ""
    for i in range(wordsize):
        ...
        command += "ASSERT(~{0}[{3}:{3}] | {1}[{3}:{3}] | {2}[{3}:{3}] = 0bin1);\n".format(a, b, c, i)
    return command
def lrotate(a, b, r, wordsize):
    command = ""
    for i in range(wordsize):
        command += "ASSERT({0}[{2}:{2}] = {1}[{3}:{3}]);\n".format(a, b, i, (i + r) % wordsize)
    return command
def rrotate(a, b, r, wordsize):
    command = ""
    for i in range(wordsize):
        command += "ASSERT({0}[{2}:{2}] = {1}[{3}:{3}]);\n".format(b, a, i, (i + r) % wordsize)
    return command
def xor_diff(a, b, c, wordsize):
    command = ""
    for i in range(wordsize):
        command += "ASSERT(~{0}[{3}:{3}] | ~{1}[{3}:{3}] | ~{2}[{3}:{3}] = 0bin1);\n".format(a, b, c, i)
        command += "ASSERT({0}[{3}:{3}] | {1}[{3}:{3}] | ~{2}[{3}:{3}] = 0bin1);\n".format(a, b, c, i)
        ...
        command += "ASSERT(~{0}[{3}:{3}] | {1}[{3}:{3}] | {2}[{3}:{3}] = 0bin1);\n".format(a, b, c, i)
    return command
def xor_lin(a, b, c, wordsize):
    command = ""
    command += "ASSERT({} = {});\n".format(a, b)
    command += "ASSERT({} = {});\n".format(a, c)
    return command
def and_diff(a, b, c, w, wordsize):
    command = ""
    for i in range(wordsize):
...
def and_lin(a, b, c, w, wordsize):
    ...
    return command
def add_diff(a, b, c, w, wordsize):
    command = ""
    command += "ASSERT(~{0}[0:0] | ~{1}[0:0] | ~{2}[0:0] = 0bin1);\n".format(a, b, c)
    command += "ASSERT({0}[0:0] | {1}[0:0] | ~{2}[0:0] = 0bin1);\n".format(a, b, c)
    command += "ASSERT({0}[0:0] | ~{1}[0:0] | {2}[0:0] = 0bin1);\n".format(a, b, c)
    command += "ASSERT(~{0}[0:0] | {1}[0:0] | {2}[0:0] = 0bin1);\n".format(a, b, c)
    for i in range(wordsize - 1):
        command += "ASSERT({0}[{4}:{4}] | ~{1}[{4}:{4}] | {3}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, i + 1)
        command += "ASSERT(~{0}[{4}:{4}] | {2}[{4}:{4}] | {3}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, i + 1)
        command += "ASSERT({0}[{4}:{4}] | {1}[{4}:{4}] | {2}[{4}:{4}] | ~{3}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, i + 1)
        command += "ASSERT(~{0}[{4}:{4}] | ~{1}[{4}:{4}] | ~{2}[{4}:{4}] | ~{3}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, i + 1)
        command += "ASSERT({1}[{4}:{4}] | ~{0}[{5}:{5}] | ~{1}[{5}:{5}] | ~{2}[{5}:{5}] | {3}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, i + 1)
        command += "ASSERT(~{2}[{4}:{4}] | {0}[{5}:{5}] | ~{1}[{5}:{5}] | ~{2}[{5}:{5}] | {3}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, i + 1)
        command += "ASSERT(~{2}[{4}:{4}] | ~{0}[{5}:{5}] | {1}[{5}:{5}] | ~{2}[{5}:{5}] | {3}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, i + 1)
        command += "ASSERT({1}[{4}:{4}] | {0}[{5}:{5}] | {1}[{5}:{5}] | ~{2}[{5}:{5}] | {3}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, i + 1)
        command += "ASSERT(~{2}[{4}:{4}] | ~{0}[{5}:{5}] | ~{1}[{5}:{5}] | {2}[{5}:{5}] | {3}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, i + 1)
        command += "ASSERT({1}[{4}:{4}] | {0}[{5}:{5}] | ~{1}[{5}:{5}] | {2}[{5}:{5}] | {3}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, i + 1)
        ...
        command += "ASSERT(~{2}[{4}:{4}] | {0}[{5}:{5}] | {1}[{5}:{5}] | {2}[{5}:{5}] | {3}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, i + 1)
    return command
def add_lin(a, b, c, w, wordsize):
    command = ""
    command += "ASSERT({}[0:0] = 0bin0);\n".format(w)
    for i in range(wordsize):

        command += "ASSERT(~{3}[{6}:{6}] | ~{2}[{4}:{4}] | ~{0}[{4}:{4}] | ~{1}[{4}:{4}] | ~{3}[{5}:{5}] = 0bin1);\n".format(a, b, c, w, i, wordsize - 1 - i, wordsize - i)

        command += "ASSERT({3}[{6}:{6}] | {2}[{4}:{4}] | {0}[{4}:{4}] | {1}[{4}:{4}] | ~{3}[{5}:{5}] = 0bin1);\n".format(a, b, c, w, i, wordsize - 1 - i, wordsize - i)
        command += "ASSERT({3}[{6}:{6}] | {2}[{4}:{4}] | ~{0}[{4}:{4}] | ~{1}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, wordsize - 1 - i, wordsize - i)
        command += "ASSERT({3}[{6}:{6}] | ~{2}[{4}:{4}] | {0}[{4}:{4}] | ~{1}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, wordsize - 1 - i, wordsize - i)
        command += "ASSERT(~{3}[{6}:{6}] | {2}[{4}:{4}] | {0}[{4}:{4}] | ~{1}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, wordsize - 1 - i, wordsize - i)
        command += "ASSERT({3}[{6}:{6}] | ~{2}[{4}:{4}] | ~{0}[{4}:{4}] | {1}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, wordsize - 1 - i, wordsize - i)
        command += "ASSERT(~{3}[{6}:{6}] | {2}[{4}:{4}] | ~{0}[{4}:{4}] | {1}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, wordsize - 1 - i, wordsize - i)
        command += "ASSERT(~{3}[{6}:{6}] | ~{2}[{4}:{4}] | {0}[{4}:{4}] | {1}[{4}:{4}] = 0bin1);\n".format(a, b, c, w, i, wordsize - 1 - i, wordsize - i)
        command += "ASSERT({2}[{4}:{4}] | ~{0}[{4}:{4}] | {3}[{5}:{5}] = 0bin1);\n".format(a, b, c, w, i, wordsize - 1 - i, wordsize - i)
        command += "ASSERT(~{2}[{4}:{4}] | {0}[{4}:{4}] | {3}[{5}:{5}] = 0bin1);\n".format(a, b, c, w, i, wordsize - 1 - i, wordsize - i)
        command += "ASSERT({3}[{6}:{6}] | ~{1}[{4}:{4}] | {3}[{5}:{5}] = 0bin1);\n".format(a, b, c, w, i, wordsize - 1 - i, wordsize - i)
        command += "ASSERT(~{3}[{6}:{6}] | {1}[{4}:{4}] | {3}[{5}:{5}] = 0bin1);\n".format(a, b, c, w, i, wordsize - 1 - i, wordsize - i)
    return command
